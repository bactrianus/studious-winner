...
public class Utils {
	...
	public static ArrayList<FileBrowserTreeNode> ToFileTreeNodes(String path, List<FileDescription> list) {
		...
		for (FileDescription nodes : list) {
			...
			tempVar.path = DotNetToJavaStringHelper.isNullOrEmpty(path) ? nodes.getName()
					: String.format("%1$s/%2$s", path, nodes.getName());
			tempVar.docType = DotNetToJavaStringHelper.isNullOrEmpty(nodes.getDocumentType())
					? nodes.getDocumentType() : nodes.getDocumentType().toLowerCase();
			tempVar.fileType = DotNetToJavaStringHelper.isNullOrEmpty(nodes.getFileType())
	...
	public static Watermark GetWatermark(String watermarkText, Integer watermarkColor,
		...
		if (DotNetToJavaStringHelper.isNullOrWhiteSpace(watermarkText)) {
			return null;
		}
		Color c = null;
